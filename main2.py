...
from datasets import VOCSegmentation, Cityscapes, PaintedSymbols
...
def get_argparser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="voc",
        choices=["voc", "cityscapes", "painted_symbols"],
...
def get_dataset(opts):
    ...
    if opts.dataset == "painted_symbols":
        train_transform = et.ExtCompose(
            [
                # et.ExtResize( 512 ),
                et.ExtRandomCrop(size=(opts.crop_size, opts.crop_size)),
        ...
        train_dst = PaintedSymbols(split="train", transform=train_transform)
        val_dst = PaintedSymbols(split="eval", transform=val_transform)
...
def main():
    ...
    if opts.dataset.lower() == "voc":
        ...
        opts.num_classes = 19
    elif opts.dataset.lower() == "painted_symbols":
        opts.num_classes = 3
    ...
    # Assuming your pretrained model is called 'pretrained_model'
    class ModifiedModel(nn.Module):
        def __init__(self, pretrained_model):
            super(ModifiedModel, self).__init__()
            self.pretrained_model = pretrained_model
            self.new_layer = nn.Conv2d(
                in_channels=19, out_channels=3, kernel_size=1
            )
    ...
    optimizer = torch.optim.SGD(
        params=[
            {"params": model.pretrained_model.backbone.parameters(), "lr": 0.1 * opts.lr},
            {"params": model.pretrained_model.classifier.parameters(), "lr": 0.3 * opts.lr},
            {"params": model.new_layer.parameters(), "lr": opts.lr}
        ],
        lr=opts.lr,
        momentum=0.9,
