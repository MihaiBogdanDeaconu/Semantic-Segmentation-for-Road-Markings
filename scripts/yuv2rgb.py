...
def main():
    input_dir = r"C:\Users\DEM7CLJ\Documents\SemsegProject\Test"
    output_dir = r"\\abtvdfs2.de.bosch.com\ismdfs\ida\abt\video\Val_FV_Gen3\01_all\SemsegCrosswalkRestrictedArea\test_rgb"

    for img_name in os.listdir(input_dir):
        input_file_path = os.path.join(input_dir, img_name)
        image = cv2.imread(input_file_path, -1)
        rgb_image = convert_luv_to_rgb(image[:, :, 0], image[:, :, [1, 2]]).numpy()[0]

        output_file_path = os.path.join(output_dir, img_name.lower())
        rgb_image = cv2.cvtColor(rgb_image, cv2.COLOR_BGR2RGB)
        cv2.imwrite(output_file_path, rgb_image)
